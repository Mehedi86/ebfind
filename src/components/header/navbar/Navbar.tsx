...
export default function Navbar() {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 w-full transition-all duration-300 ${
        ...
        <div className="flex items-center justify-between h-16 md:h-20 w-full">
          ...
          <div className="flex items-center space-x-2 shrink-0">
            <Link href="/" className="flex items-center space-x-2 group">
              <div className={`p-2 rounded-lg transition-colors shadow-lg ${
                scrolled 
                  ? 'bg-red-600 group-hover:bg-red-700' 
                  : 'bg-white/90 backdrop-blur-sm group-hover:bg-white'
              }`}>
                <Droplet className={`h-6 w-6 transition-colors ${
          ...
          <div className="hidden md:flex items-center justify-center flex-1 space-x-1 lg:space-x-4 mx-4 lg:mx-8">
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className={`px-3 py-2 text-sm lg:text-base font-medium rounded-md transition-all whitespace-nowrap ${
                  scrolled
      ...
      >
        <div className={`px-4 pt-2 pb-4 space-y-1 border-t transition-colors ${
          scrolled
